...
#             "depth": norm_disparity,
    ...
    # max_per_pixel = rendered_image.max(dim=0)[0]
    # mask = max_per_pixel > 1
    # rendered_image[:, mask] = rendered_image[:, mask] / max_per_pixel[mask]
    
    # mask = (alpha <= 0.6).expand_as(rendered_image)
    # min_val = rendered_image[mask].min() if torch.any(mask) else rendered_image.min()
    # max_val = rendered_image[mask].max() if torch.any(mask) else rendered_image.max()
    # rendered_image[mask] = (rendered_image[mask] - min_val) / (max_val - min_val)
